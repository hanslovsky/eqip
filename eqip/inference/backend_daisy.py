...
def predict_affinities_daisy():
    ...
    parser.add_argument('--network-output-shape', nargs=3, type=int, default=(209, 214, 214), help='zyx')
    ...
    parser.add_argument('--output-placeholder-tensor', default='Reshape_1:0')
    ...
    with z5py.File(output_container, use_zarr_format=False) as f:
        ds = f.require_dataset(
            ...
            dtype=np.float32,
            chunks = (num_channels,) + tuple(network_output_shape) if num_channels > 0 else tuple(network_output_shape),
            compression='raw')
